...
use crate::config::{TimedConfig, get_default_config_path};
...
pub fn view_config(config: &TimedConfig, config_path: &Path) -> Result<()> {
    ...
    println!("Authentication Token: {}", if config.has_token() { "Present" } else { "Not present" });
...
pub fn view_config_without_loading(config_path: &Path) -> Result<()> {
    if !config_path.exists() {
        return Err(anyhow::anyhow!("Configuration file does not exist at {}", config_path.display()));
    }
    
    let config_text = std::fs::read_to_string(config_path)
        .context("Failed to read configuration file")?;
    
    let config: TimedConfig = toml::from_str(&config_text)
...
pub fn set_config_without_loading(
    config_path: &Path,
    key: &str, 
    value: &str
) -> Result<()> {
    debug!("Setting configuration {} = {} (without loading)", key, value);
...
pub fn init_config(config_path: &Path) -> Result<()> {
    ...
    println!("Created new configuration file at {}", config_path.display());
