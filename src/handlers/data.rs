        ...
        match output_format {
            ...
            "csv" => {
                ...
                for customer in customers {
                    ...
                    let archived = customer["attributes"]["archived"].as_bool().unwrap_or(false);
                    println!("{},{},{}", id, name, archived);
                }
            },
            "text" => {
        ...
        match output_format {
            ...
            "csv" => {
                ...
                for project in projects {
                    ...
                    let customer_id = project["relationships"]["customer"]["data"]["id"].as_str().unwrap_or("");
                    println!("{},{},{},{}", id, name, customer_id, archived);
                }
            },
            "text" => {
                ...
                for project in projects {
                    ...
                    let archived = project["attributes"]["archived"].as_bool().unwrap_or(false);
                    let customer_id = project["relationships"]["customer"]["data"]["id"].as_str().unwrap_or("");
                    println!("{}: {} (customer: {}, archived: {})", id, name, customer_id, archived);
                }
            },
            _ => {
                return Err(anyhow::anyhow!("Invalid output format: {}", output_format));
...
pub async fn get_tasks(
    ...
    if let Some(id) = project_id {
        ...
        if let Some(c_id) = customer_id {
            project_filter.custom.insert("customer".to_string(), c_id.to_string());
            ...
            if let Some(customers) = customer_response["data"].as_array() {
                ...
                project_filter.custom.insert("customer".to_string(), id.to_string());
            }
        }
        ...
        if let Some(projects) = project_response["data"].as_array() {
            if !projects.is_empty() {
                let project_id = projects[0]["id"].as_str()
                    .ok_or_else(|| anyhow::anyhow!("Invalid project ID"))?;
        ...
        if let Some(customers) = customer_response["data"].as_array() {
            ...
            let mut project_filter = FilterParams::default();
            project_filter.custom.insert("customer".to_string(), customer_id.to_string());
        ...
        match output_format {
            ...
            "csv" => {
                ...
                for task in tasks {
                    ...
                    let project_id = task["relationships"]["project"]["data"]["id"].as_str().unwrap_or("");
                    println!("{},{},{},{}", id, name, project_id, archived);
                }
            },
            "text" => {
                ...
                for task in tasks {
                    ...
                    let archived = task["attributes"]["archived"].as_bool().unwrap_or(false);
                    let project_id = task["relationships"]["project"]["data"]["id"].as_str().unwrap_or("");
                    println!("{}: {} (project: {}, archived: {})", id, name, project_id, archived);
                }
            },
            _ => {
                return Err(anyhow::anyhow!("Invalid output format: {}", output_format));
