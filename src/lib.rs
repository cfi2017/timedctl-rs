...
impl TimedClient {
    ...
    pub async fn get<T: for<'de> Deserialize<'de>>(
        ...
        req = req.header(header::CONTENT_TYPE, "application/vnd.api+json");
        ...
        let data = response.json::<T>().await.map_err(|e| {
            TimedError::Serialization(serde_json::Error::io(std::io::Error::new(
...
mod tests {
    ...
    fn test_api_resource_paths() {
        ...
        assert_eq!(
            ApiResource::ProjectStatistics.as_path(),
            "project-statistics"
        );
        assert_eq!(
            ApiResource::CustomerStatistics.as_path(),
            "customer-statistics"
        );
